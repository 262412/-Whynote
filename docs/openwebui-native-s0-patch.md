...
本分支最新开发侧状态：用户已签署 Q-16 关联规则，可信关联及事务修复已实现并复验，详见 [Q-16 交付记录](q16-feedback-ownership.md)。以下独立 QA 结论保留为修复前基线，尚无新的独立验收结论。

最新独立 QA（2026-09-26）：main `db6d5b1` 的 Q-12～Q-14 原复现范围局部通过；按技术文档 H-02 新增的 **Q-16 仍失败**——未经验证的客户端 chat_id 可导致跨用户评分清理。扩展原生检查 55/56，完整删除范围不通过；详见 [本轮复验](qa-q11-q15-retest.md)。下述正常浏览器证据属于开发侧，不是测试侧本轮独立浏览器复测。

状态：**本分支已修复 Q-12/Q-13/Q-14，真实路由/SQLite 回归及正常浏览器评分路径通过，待非作者及责任人复验签署**。2026-09-26，基线 main `86f21b7`，PRD 修订 68、技术文档修订 12；见 [S0 评审修复](s0-review-fixes.md)。[旧 QA 报告](qa-pr10-11.md)保留修复前证据。对应 D-11～D-13、TD-04/06、Q-05。原生评分复用仍为 NO-GO；评分及内置原因不是知因事件来源或独立 gold。

## 固定构建与数据边界

后续阻断：PRD 80、技术文档 20 的 Q-16/H-02 仍不通过。在 main `63a0c54` 的固定补丁上重新执行独立扩展回归为 **55/56**：Bob 伪造 Alice 的 chat_id 被接受，Alice 删除聊天时 Bob 的评分被误删。当前 24 项 CI 的通过不能关闭此缺口。这属于修复前基线。[关联契约](q16-association-contract-proposal.md)已由用户签署，本分支完成创建/更新与可信删除修复；最新开发侧证据和迁移说明见 [Q-16 交付](q16-feedback-ownership.md)，独立 QA 尚未关闭 H-02。
...
3. 当 `ENABLE_ADMIN_EXPORT=false` 时，管理员评分导出、列表、ID 列表返回 401。评分 GET/POST 均只允许所有者（含管理员本人），非所有者返回 404 且不修改评分。开关开启时保留上游管理员读写权限；管理员 DELETE 管理权限未重定义。宿主管理员和数据库操作者仍控制实例，开关不是物理隔离。
4. 删除聊天、用户全部聊天或文件夹聊天时，仅清理服务端验证版本为 1、引用一致且作者仍为实际聊天所有者的评分；未验证及矛盾记录保留并登记审核。删除用户时还删除该作者全部评分，聊天、评分、审核任务、用户及登录凭据的删除统一提交或回滚。客户端 `meta.chat_id` 本身不构成删除依据。SQLite 连接启用 `secure_delete=ON`；本轮实例另设 `DATABASE_ENABLE_SQLITE_WAL=false`。这改善活动库的可观察残留，不擦除已生成的备份、导出、文件系统或设备层历史副本。
...
以下表格为 PR #11 的历史观测；其中管理员跨用户 GET 的 401 已在本分支按所有者隔离统一为 404，新权限矩阵与复验命令见 [修复记录](s0-review-fixes.md)。历史复核使用 `.invalid` 测试身份、自编中文虚构聊天和本机浏览器；未导入公开数据、生产对话或真实模型：
